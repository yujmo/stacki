...
class Command(stack.commands.add.firmware.command, stack.commands.HostArgumentProcessor):
	"""
	Maps firmware files to hosts so 'stack sync host firmware' can find them.

	<arg type='string' name='host'>
	The hostnames to associate with a firmware version.
	</arg>
	...
	<example cmd="add firmware mapping switch-11-13 version=3.6.2010 make=mellanox model=m7800">
	Maps firmware version 3.6.2010 for make mellanox and model m7800 to the host with the name switch-11-3.
	</example>

	<example cmd="add firmware mapping switch-11-13 switch-10-15 version=3.6.2010 make=mellanox model=m7800">
	This is the same as the previous example except the firmware will be mapped to both switch-11-3 and switch-10-15.
	</example>

	<example cmd="add firmware mapping a:switch version=3.6.2010 make=mellanox model=m7800">
