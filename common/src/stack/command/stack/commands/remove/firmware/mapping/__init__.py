...
class Command(stack.commands.remove.firmware.command, stack.commands.HostArgumentProcessor):
	"""
	...
	<example cmd="remove firmware mapping">
	Removes all firmware mappings for all hosts.
	</example>

	<example cmd="remove firmware mapping switch-13-11 switch-13-12 versions=3.6.5002,3.6.8010 make=mellanox model=m7800">
	...
	<example cmd="remove firmware mapping make=mellanox model=m7800">
	Unmaps all firmware for the mellanox make and m7800 model from all hosts.
	</example>

	<example cmd="remove firmware mapping switch-13-11 make=mellanox">
	Unmaps all of the firmware for the mellanox make from the host named switch-13-11.
	</example>

	<example cmd="remove firmware mapping make=mellanox">
